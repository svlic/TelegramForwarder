...
class SummaryScheduler:
    ...
    async def _execute_summary(self, rule_id, is_now=False, lookback_hours=24):
        ...
        with get_db_session() as session:
            ...
            if not rule:
                return
            if not is_now and not rule.is_summary:
                return
            ...
            is_top_summary = rule.is_top_summary
        ...
        try:
            ...
            logger.info(f"使用规则配置的AI模型进行总结: {ai_model}")
            ...
            if summary:
                ...
                header = f"📋 {source_chat_name} - {duration_hours}小时消息总结\n"
                ...
                for i, part in enumerate(summary_parts):
                    if i == 0:
                        ...
                        message_to_send = f"📋 {source_chat_name} - 总结报告 (续 {i+1}/{len(summary_parts)})\n\n" + part
                ...
                if is_top_summary and summary_message:
                    try:
